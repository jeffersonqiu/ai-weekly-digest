...
def check_data():
    ...
    try:
        ...
        for run in runs:
            print(
                f"Run ID: {run.id}, Status: {run.status}, Papers Count: {run.papers_count}, Created: {run.created_at}"
            )
        ...
        scores = db.scalars(
            select(PaperScore).order_by(PaperScore.final_score.desc()).limit(5)
        ).all()
        if scores:
            for score in scores:
                print(
                    f"Paper ID: {score.paper_id}, Final: {score.final_score:.2f} (Auth: {score.author_score:.2f}, Cat: {score.category_score:.2f}, L: {score.llm_interest_score:.2f})"
                )
        else:
        ...
        if test_paper and test_paper.score:
            ...
            print(
                f"Final: {s.final_score:.2f} (Auth: {s.author_score:.2f}, Cat: {s.category_score:.2f}, L: {s.llm_interest_score:.2f})"
            )
