...
def classify_paper_heuristic(paper: Paper) -> str:
    ...
    if any(
        k in text
        for k in ["evaluation", "benchmark", "metric", "survey", "analysis", "dataset"]
    ):
...
def generate_category_chart(run_id: int, db, output_path: str) -> str:
    ...
    norm = mcolors.Normalize(
        vmin=0, vmax=max(plot_values) * 1.2
    )  # Normalize based on max count
    ...
    # Title and labels
    ax.set_title(
        f"Paper Distribution by Category (Total: {total_papers})",
        fontsize=16,
        fontweight="bold",
        pad=20,
        color="#333333",
...
def generate_digest():
    ...
    try:
        ...
        stmt = (
            ...
            .where(PaperScore.rank.is_not(None))
        ...
        if not scores:
            logger.error(
                f"No ranked papers found for Run {run.id}. Did you run rank_papers.py?"
            )
            return
...
"""
        ...
        existing_digest = db.execute(
            select(Digest).where(Digest.run_id == run.id)
        ).scalar_one_or_none()
        if existing_digest:
        ...
        digest_obj = Digest(run_id=run.id, markdown=final_digest, html=None)
        db.add(digest_obj)
        db.commit()
