...
class DatasetBuilder:
    ...
    def fetch_historical_papers(
        self, days_ago_start: int, days_ago_end: int, max_papers: int = 500
    ) -> List[ArxivPaper]:
    ...
    def get_citation_counts(self, papers: List[ArxivPaper]) -> Dict[str, int]:
        ...
        for i in range(0, len(search_ids), batch_size):
            ...
            while retries > 0:
                try:
                    logger.info(
                        f"Processing Semantic Scholar batch {current_batch_num}/{total_batches}..."
                    )
                    results = self.sch.get_papers(
                        batch, fields=["citationCount", "title", "externalIds"]
                    )
    ...
    def save_to_csv(
        self, papers: List[ArxivPaper], citation_map: Dict[str, int], filename: str
    ):
        ...
        logger.info(
            f"Saved chunk. Found citation data for {count_with_citations}/{len(papers)} papers."
        )
...
def main():
    ...
    while current_days_ago > end_days_ago:
        ...
        chunk_filename = os.path.join(
            chunks_dir, f"papers_{chunk_start}_{chunk_end}.csv"
        )
        ...
        try:
            ...
            papers = builder.fetch_historical_papers(
                days_ago_start=chunk_start, days_ago_end=chunk_end, max_papers=5000
            )
